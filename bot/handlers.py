...
class BotHandlers:
    ...
    async def _handle_payment_selection(self, query, data):
        ...
        if data == "payment_cash_card":
            await self._handle_card_to_card_payment(query, "cash",
                                                    "پرداخت نقدی", 0.30)
        elif data == "payment_60day_card":
            await self._handle_card_to_card_payment(query, "60day",
                                                    "پرداخت ۶۰ روز", 0.25)
        elif data == "payment_90day_card":
            await self._handle_card_to_card_payment(query, "90day",
                                                    "پرداخت ۹۰ روز", 0.25)
        elif data == "payment_check_60day":
            await self._handle_check_payment(query, "60day",
                                             "پرداخت چکی 60 روزه", 0.25)
    ...
    async def _handle_card_to_card_payment(self, query, payment_type: str,
        ...
        if payment_type == "cash":
            ...
            half_amount = int(final_amount / 2)
            payment_details = (
                ...
                f"درصورت پرداخت به صورت چکی ،تاریخ سر رسید چک در همین دوماه باشد.و چک را به کارخانه ارسال کنید ")
            ...
            keyboard = [[
                InlineKeyboardButton("💰 پرداخت نقدی ",
                        ...
                        [
                            InlineKeyboardButton(
                                "📋 پرداخت و ثبت چکی",
                                callback_data="payment_check_60day")
                        ],
                        [
                            InlineKeyboardButton("🔙 بازگشت",
                                                 callback_data="view_invoice")
            ...
            payment_details = (
                ...
                f"🏦 شماره شبا: IR110560611828005185959401\n"
                f"👤 به نام: نیما کریمی\n\n"
                f"📸 پس از واریز پیش ‌پرداخت/ ثبت چک لطفاً عکس آن را ارسال کنید درصورت پرداخت به صورت چکی ،چک رابه کارخانه ارسال کنید:"
            )
            # Create keyboard for 90-day payment
            keyboard = [[
                InlineKeyboardButton("📸 ارسال فیش /چک ثبت شده",
                                     callback_data="upload_receipt")
            ], [
                InlineKeyboardButton("🔙 بازگشت", callback_data="view_invoice")
            ]]
